...
const App = {
  ...
  filterPosts(){
    ...
    if(filter!=='all'&&filter!=='following')posts=posts.filter(p=>(p.tags||[]).includes(filter));
  ...
  renderProfilePanel(){
    const panel=U.$('#profilePanel');if(!panel)return;
...
/* ═══ AUTH ═══ */
const Auth = {
  open(){U.$('#authModal').classList.add('open');document.body.style.overflow='hidden'},
  close(e){if(e&&e.target!==e.currentTarget)return;U.$('#authModal').classList.remove('open');document.body.style.overflow=''},
  async signIn(){
