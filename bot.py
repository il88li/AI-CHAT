...
async def main():
    try:
        ...
        try:
            ...
            status = "مفعل" if is_bot_enabled() else "متوقف"
            vip_status = "مفعل" if is_vip_mode() else "معطل"
            await app.send_message(
                ADMIN_ID, 
                f"🤖 البوت @{bot_info.username} يعمل الآن!\n"
                f"🔧 الحالة: {status}\n"
                f"🎯 وضع VIP: {vip_status}"
            )
        except:
            print("⚠️ لا يمكن إرسال رسالة للمدير")
        ...
        for user in users:
            if user[3] and user[1]:  # إذا كان الرد التلقائي مفعل وهناك جلسة
                try:
                    ...
                    print(f"❌ خطأ في تشغيل جلسة المستخدم {user[0]}: {e}")
        
        print(f"✅ تم تشغيل {active_count} جلسة نشطة")
        await idle()
