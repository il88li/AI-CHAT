...
# Keep Alive System for webhook requests
class KeepAliveSystem:
    ...
    async def start_keep_alive(self):
        ...
        while self.is_active:
            try:
                # Send request to webhook to keep service active
                async with httpx.AsyncClient() as client:
                    response = await client.get(self.webhook_url, timeout=30.0)
...
keep_alive_system = KeepAliveSystem(WEBHOOK_URL)
...
async def main():
    ...
    print("✅ Bot is running with Telegram session management and keep-alive system...")
    await app.start()
