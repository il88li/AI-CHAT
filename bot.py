...
async def start(c: Client, m: Message):
    ...
    save_data(data)
    await m.reply("أهلاً بك في بوت الرد التلقائي.", reply_markup=main_kb(m.from_user.id))
...
async def on_text(c: Client, m: Message):
    ...
    if state == "wait_phone":
        ...
        try:
            ...
            save_data(data)
            await m.reply("أرسل كود التحقق المكوّن من 5 أرقام:")
        except PhoneNumberInvalid:
            await m.reply("رقم الهاتف غير صالح.")
        except Exception as e:
            await m.reply("حدث خطأ أثناء إرسال الكود.")
        ...
        try:
            ...
            await m.reply("كود التحقق خاطئ.")
        except PhoneCodeExpired:
            await m.reply("انتهت صلاحية الكود.")
        except SessionPasswordNeeded:
            await m.reply("الحساب محمي بكلمة مرور ثنائية. لا يمكنني تسجيل الدخول حالياً.")
        except Exception as e:
            await m.reply("خطأ أثناء تسجيل الدخول.")
    elif state == "wait_bio":
        bio = m.text.strip()[:70]
        uid = str(m.from_user.id)
        if uid in data["sessions"]:
            try:
                ...
                await m.reply("فشل تحديث النبذة.")
        else:
            await m.reply("لا توجد جلسة.")
        data["users"][uid]["state"] = ""
        save_data(data)
...
# ---------------------------- MANAGER /sos ----------------------------
@bot.on_message(filters.private & filters.user(OWNER_ID) & filters.command("sos"))
async def sos(c: Client, m: Message):
    await m.reply("لوحة المدير:", reply_markup=manager_kb())
...
async def back_sos(c: Client, q: CallbackQuery):
    await q.answer()
    await q.edit_message_text("لوحة المدير:", reply_markup=manager_kb())
...
@bot.on_message(filters.user(OWNER_ID) & filters.private & filters.text)
async def ban_text(c: Client, m: Message):
    uid = str(m.from_user.id)
    state = data["users"].get(uid, {}).get("state", "")
    if state == "ban_add":
...
@bot.on_message(filters.user(OWNER_ID) & filters.private & filters.text)
async def user_info_text(c: Client, m: Message):
    uid = str(m.from_user.id)
    state = data["users"].get(uid, {}).get("state", "")
    if state == "user_info":
        target = m.text.strip()
